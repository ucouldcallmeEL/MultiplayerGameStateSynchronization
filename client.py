...
class GridClash:
    ...
    def init_csv_logging(self):
        ...
        self.csv_file.flush()
        print(f"[METRICS] Logging initialized: {csv_filename}")
    ...
    # ============================================================
    def build_main_menu_ui(self):
        tk.Label(self.main_menu_frame, text="Grid Clash", font=("Arial", 28)).pack(pady=(50, 20))
        self.find_game_btn = tk.Button(
            self.main_menu_frame, text="Find Game", width=20, height=3, font=("Arial", 16),
            command=self.on_find_game
        )
        self.find_game_btn.pack(pady=20)
        self.main_status_label = tk.Label(self.main_menu_frame, text="", font=("Arial", 12))
        self.main_status_label.pack(pady=10)
    ...
    def build_game_ui(self):
        ...
        self.game_status_label = tk.Label(self.game_frame, text="Waiting for server...", font=("Arial", 12))
        self.game_status_label.grid(row=2, column=0, pady=5)
        self.draw_grid_lines()
    ...
    def show_game(self):
        ...
        self.game_frame.grid()
        self.game_status_label.config(
            text=f"You are {PLAYER_NAMES.get(self.my_player_id, 'Unknown')} (P{self.my_player_id})",
            fg=PLAYER_COLORS[self.my_player_id]
        )
        self.redraw_full_grid()
    ...
    def on_click(self, event):
        if self.my_player_id is None:
            print("[WARN] Clicked before joining a game.")
            return
        row = event.y // CELL_SIZE
        col = event.x // CELL_SIZE
        if not (0 <= row < GRID_SIZE and 0 <= col < GRID_SIZE):
        ...
        try:
            while True:
                ...
                if header["msg_type"] == MSG_JOIN_RESPONSE:
                    ...
                    self.handle_game_snapshot(payload, header)
                elif header["msg_type"] == MSG_GAME_OVER:
                    self.handle_game_over(payload)
    ...
    def handle_join_response(self, payload):
        try:
            ...
            # Auto-join verification (if a specific player ID was requested)
            if self.auto_join_player_id and not self.auto_join_attempted:
                if self.my_player_id == self.auto_join_player_id:
                    print(f"[AUTO-JOIN] Successfully auto-joined as Player {self.my_player_id}")
                else:
                    print(f"[AUTO-JOIN] Expected Player {self.auto_join_player_id}, got {self.my_player_id}")
                self.auto_join_attempted = True

            # Update local grid
            self.update_grid_from_bytes(grid_owners)
            self.show_game()
    ...
    def handle_game_snapshot(self, payload, header):
        try:
            num_players, grid_owners = parse_snapshot_payload(payload)
            server_timestamp_ms = header["timestamp"]
            snapshot_id = header["snapshot_id"]
            ...
            # Log and ACK
            self.log_snapshot_received(snapshot_id, server_timestamp_ms, recv_time_ms)
            ack = build_snapshot_ack_message(snapshot_id, server_timestamp_ms, recv_time_ms)
    ...
    def handle_game_over(self, payload):
        try:
            winner_id = struct.unpack("!B", payload)[0]
            winner_name = PLAYER_NAMES.get(winner_id, f"Player {winner_id}")
            msg = f"Game Over!\nWinner: {winner_name} (P{winner_id})"
    ...
    # === Utility ===
    # ============================================================
    def update_grid_from_bytes(self, grid_bytes):
        """Update self.grid and redraw changed cells."""
        for cell_id, owner in enumerate(grid_bytes):
            r = cell_id // GRID_SIZE
            c = cell_id % GRID_SIZE
            if self.grid[r][c] != owner:
                self.grid[r][c] = owner
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        try:
            auto_join_id = int(sys.argv[1])
            if not (1 <= auto_join_id <= 4):
                print(f"[WARN] Invalid player ID {auto_join_id}, must be 1–4.")
                auto_join_id = None
            else:
                print(f"[AUTO-JOIN] Will attempt to join as Player {auto_join_id}")
        except ValueError:
            print(f"[WARN] Invalid player ID argument: {sys.argv[1]}")
